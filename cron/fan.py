...
class FanManager:
    def __init__(self):
        ...
        # is the fan spinning now
        self.spinning = open("/sys/class/pwm/pwmchip1/pwm0/enable", "r").read().strip() == "1" 
        # start spinning when curr_temp > start_temp
        self.start_temp = 36  
        # stop spinning when temp < stop_temp
        self.stop_temp = 32 
        # try to get temp to this value
        self.target_temp = 34  

        self.period = 100
        # actual spin speed lower bound
        self.min_speed = 0.4
        # actual spin speed upper bound
        self.max_speed = 1
        curr_duty_cycle = int(open("/sys/class/pwm/pwmchip1/pwm0/duty_cycle", "r").read().strip())
        curr_period = int(open("/sys/class/pwm/pwmchip1/pwm0/period", "r")).read().strip()
        self.curr_speed = (curr_duty_cycle / curr_period - self.min_speed) / (self.max_speed - self.min_speed)
        # change step each update
        self.step = 0.1
    ...
    def start(self):
        ...
        while True:
            ...
            print(
                ...
                temp,
                self.spinning,
                "{:.2f}".format(curr_speed),
                duty_cycle if self.spinning else "Na",
                file=log_file,
            )
            cache_count -= 1
            if cache_count == 0:
